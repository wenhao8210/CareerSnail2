        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full mt-2 py-1.5 min-w-[160px] rounded-xl border border-white/10 shadow-xl z-50" style={{ backgroundColor: "rgba(26,26,26,0.98)" }}>
              ...
              <Link href="/interview-notes" className="block px-4 py-3 text-sm font-medium text-gray-200 hover:bg-white/10 transition rounded-b-xl" onClick={() => setMenuOpen(false)} style={{ color: ACCENT }}>面试复盘</Link>
            </div>
          )}
        </div>
      </header>
                ...
                <div className="inline-flex min-w-full min-h-full py-3 px-6" style={{ minWidth: dateKeys.length * PX_PER_DAY + 48 }}>
                  {dateKeys.map((dateKey) => {
                    ...
                    >
                      ...
                      <div
                        className={`flex-1 flex flex-col px-1 min-h-[280px] pt-[30px] transition-colors cursor-context-menu ${isToday ? "bg-[#C2319A]/5" : ""} ${dragOverDateKey === dateKey ? "bg-white/10 rounded-lg" : ""}`}
